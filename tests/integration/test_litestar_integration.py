"""Integration test driving a real in-process uvicorn + Litestar server.

Marked ``integration`` so it does NOT run under ``-m unit``. It exercises the
actual Litestar + uvicorn stack (no mocking): a real ``uvicorn.Server`` is bound
...
async def test__litestar_entrypoint__real_server__serves_requests_then_drains() -> None:
    port = _free_port()
    container = FakeContainer()
    spec: AppSpec[Any, Any] = AppSpec(service_name="litestar-it-service", create_container=lambda _s: container)

    ep = LitestarEntrypoint(config=LitestarConfig(host="127.0.0.1", port=port, graceful_timeout=2.0))
    ...
    async def probe_then_stop() -> None:
        ...
        async with httpx.AsyncClient(base_url=base_url, timeout=2.0) as client:
            for _ in range(200):  # ~2s max startup wait
                try:
                    resp = await client.get("/system/readyz")
